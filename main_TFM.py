...
msg = []
command = []
#roi = (400,2800,700,3100)
#roi = (750,3500,450,3200) #play2 Rotated
roi = (450,3200,1150,3900) #plays2
display = False
turn_pl =  True
ang = 0
dim = (720,720)
...
print("Loading model...")
vggmodel = LoadCNN('models/vggmodel19_cam2_v1.h5')
clf1 = joblib.load('models/gridsearch_tfm2_1.pkl')
...
#webcam, im_msg, str_msg, board_coord, grid, center = initCamera(0,0)
webcam, im_msg, str_msg, board_coord, grid, center = loadCAMBoard(0,0)

#plt.figure("Finding Board")
#plt.imshow(im_msg)
#plt.show()
...
while not board.is_game_over():
    ...
    if turn_pl:
        ...
        while not legal_move:
            ...
            pred_board, str_board, grid = predictBoard(grid, vggmodel, clf1)

            try:
                board, move_pl, info, legal_move = playUser(board, engine, str_board, legal_move, grid)
            ...
            #    legal_move = False
            #    print("Please retry the movement\n")
            #    continue
            for mv in move_pl:
    ...
    #dividedBoard(im_msg, boardcoord.angle,  grid, center, vggdim, False)
